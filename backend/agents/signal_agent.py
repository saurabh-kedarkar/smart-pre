...
class SignalAgent:
    ...
    def generate_signal(self, symbol: str, price: float,
        ...
        # Confidence = how sure we are about the signal direction
        # Can go BELOW 50% for weak/conflicting signals
        if abs(composite) > 0.3:
            confidence = 0.55 + abs(composite) * 0.45  # Strong: 55-98%
        elif abs(composite) > 0.15:
            confidence = 0.45 + abs(composite) * 0.35  # Moderate: 45-55%
        else:
            confidence = 0.25 + abs(composite) * 0.5   # Weak: 25-45%
        ...
        if signal_type in ("BUY", "WEAK_BUY"):
            entry = price
            # Wider Targets for better profitability (R:R 1.8:1 to 6:1)
