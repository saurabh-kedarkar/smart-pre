...
class DataAgent:
    ...
    async def start_streaming(self) -> None:
        ...
        for symbol in self.symbols:
            ...
            await self.ws_manager.subscribe_kline(
                symbol, "1m", lambda data, s=symbol: self._on_kline(s, data, "1m")
            )
            await self.ws_manager.subscribe_kline(
                symbol, "15m", lambda data, s=symbol: self._on_kline(s, data, "15m")
            )
    ...
    def _on_kline(self, symbol: str, data: dict, timeframe: str = "1m") -> None:
        ...
        if symbol in self._candles and timeframe in self._candles[symbol]:
            ...
            if len(df) > 0:
                ...
                if ts in df.index:
                    ...
                    self._candles[symbol][timeframe] = pd.concat([df, new_df]).tail(500)
    ...
    def get_history(self, symbol: str, timeframe: str = "15m") -> list:
        """Get historical candles for chart display."""
