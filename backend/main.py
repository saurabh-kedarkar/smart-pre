...
def json_serializable(obj):
    """Recursively convert object fields to JSON-serializable types."""
...
def safe_json_dumps(data: Any) -> str:
    """Safely dump data to JSON string by converting NumPy/Pandas types."""
    return json.dumps(json_serializable(data))
...
async def run_backtest(symbol: str):
    """Run historical simulation for a symbol."""
    symbol = symbol.upper()
    df = data_agent.get_candles(symbol, "15m")
    if df is None or len(df) < 100:
        return {"error": "Not enough data for backtest"}
        
    backtester = BacktestAgent(initial_balance=10000.0)
    report = backtester.run_backtest(df, symbol)
    return report
